 ...
 * availability, quantity and the primary action.
 */
export function ProductCard({ product: p, view }: { product: CatalogProduct; view: "grid" | "list" }) {
  const [quantity, setQuantity] = useState(1);
  const unit = casePriceForQuantity(p, quantity);
  const firstBreak = p.volumeBreaks[0];
  ...
  return (
    ...
    >
      ...
      >
        <Link href={href} aria-label={`View ${p.name}`} className="relative block h-full w-full">
          <Image
            ...
            alt={p.name}
            fill
            sizes={list ? "192px" : "(min-width: 1280px) 25vw, (min-width: 640px) 50vw, 100vw"}
            className={cn("object-contain", list ? "p-3" : "p-6")}
          />
        </Link>
        {p.tag ? (
      ...
      <div className={cn("flex flex-1 flex-col gap-1 p-4", list && "sm:flex-row sm:items-center sm:gap-6")}>
        ...
        <div className={cn("mt-3 border-t border-border pt-3", list && "sm:mt-0 sm:w-56 sm:border-t-0 sm:pt-0")}>
        ...
        <div className={cn("mt-3 flex items-center justify-between gap-2", list && "sm:mt-0 sm:flex-col sm:items-end")}>
          <QuantityInput value={quantity} onChange={setQuantity} label={`Cases of ${p.name}`} />
          <ButtonLink href={`${href}?cases=${quantity}`} size="sm">
            View details
